...
async def handle_events(info: Info, block_events: NewEvents):
    ...
    events = [decode(e1, e2) for (e1, e2) in zip(new_simulations, end_summaries)]

    # Insert multiple documents in one call.
    await info.storage.insert_many("mumu-s2-daw-events", events)
...
async def run_indexer(server_url=None, mongo_url=None, restart=None):
    ...
    contract_address = (
        "0x04a80d0a8ce30aa76103693f41aeb265d2cb7bd164c52f262b571f42aa572d04"
    )
